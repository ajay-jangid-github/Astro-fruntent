...
const DashboardOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      const { data } = await axios.get('https://astrology-backend-1.onrender.com/api/admin/orders', {
  ...
  const updateOrderStatus = async (orderId, status) => {
    try {
      await axios.patch(`https://astrology-backend-1.onrender.com/api/admin/orders/${orderId}`, 
        { status }, 
        { headers: getAuthHeaders() }
      );
